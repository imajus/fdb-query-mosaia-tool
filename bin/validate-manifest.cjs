...
function validateToolMetadataName(name) {
  ...
  if (typeof name !== 'string' || name.length < 5) {
    ...
    errors.push(
      `\`name\` must conform to the following regex: ${validName.toString()}`
    );
  }
  ...
  if (
    ...
    if (
      ...
      if (
        ...
        errors.push(
          '`schema.function.description` must be a string (min length 30)'
        );
      }
      ...
      if (
        ...
        errors.push(
          '`schema.function.strict` must be boolean (optional value)'
        );
      }
      ...
      if (
        ...
        try {
          ...
          errors.push(
            `\`schema.function.parameters\` has the following errors: ${error.message}`
          );
        }
      }
    }
...
const isArrayOfStrings = (arr) =>
  arr.filter((v) => typeof v === 'string').length === arr.length;
